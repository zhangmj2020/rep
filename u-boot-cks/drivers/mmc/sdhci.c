...
static int sdhci_transfer_data(struct sdhci_host *host, struct mmc_data *data)
{
	...
	timeout = 4000000;  //In fpga, transfer speed is slow, so wait more time, wangpeng@20191220
...
#if defined(CONFIG_TARGET_FPGA_CKS_C01) || defined(CONFIG_TARGET_FPGA_CKS_A53T28)
	if(mmc->clock >= CONFIG_CKS_FPGA_SDCLK && host->index == 0)
	{
		mmc->clock = CONFIG_CKS_FPGA_SDCLK;
		printf("%s:cheat sdclk to %d at host %d\n",__FUNCTION__, mmc->clock, host->index);
	}
#endif
