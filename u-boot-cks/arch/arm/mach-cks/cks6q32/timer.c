...
#define CONFIG_SYS_TIMER_CLK	25000000  //periphclk 25m

static struct globaltimer *global_timer = \
		(struct globaltimer *)GTIM_BASE_ADDR;
...
static u64 get_time_us(void)
{
	...
	do_div(timer, CLK2MHZ(CONFIG_SYS_TIMER_CLK));
	return timer;
}
...
void __udelay(unsigned long usec)
{
	...
	wait = (u64)(usec * CLK2MHZ(CONFIG_SYS_TIMER_CLK));
	do {
